...
- Book 5: Prompt and Context Engineering, chapter 1
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Instructions That Work* (normal, boundary, adversarial) in `specs/lab-0501.yaml` or as pytest cases in `test_lab.py`. Goal: Solve one task with weak and strong prompts and compare failures.
...
```text
Read specs/lab-0501.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 5.1 — Instructions That Work
```
...
```text
/opsx:apply
python -m pytest labs/0501-*/test_lab.py -q
/opsx:archive
```
