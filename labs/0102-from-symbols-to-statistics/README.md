...
- Book 1: Foundations of Intelligence, chapter 2
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *From Symbols to Statistics* (normal, boundary, adversarial) in `specs/lab-0102.yaml` or as pytest cases in `test_lab.py`. Goal: Implement a tiny rule engine and document where it becomes brittle.
...
```text
Read specs/lab-0102.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 1.2 — From Symbols to Statistics
```
...
```text
/opsx:apply
python -m pytest labs/0102-*/test_lab.py -q
/opsx:archive
```
