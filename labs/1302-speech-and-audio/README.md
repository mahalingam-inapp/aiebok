...
- Book 13: Multimodal and Frontier Systems, chapter 2
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Speech and Audio* (normal, boundary, adversarial) in `specs/lab-1302.yaml` or as pytest cases in `test_lab.py`. Goal: Build a transcript pipeline with timestamps and confidence handling.
...
```text
Read specs/lab-1302.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 13.2 — Speech and Audio
```
...
```text
/opsx:apply
python -m pytest labs/1302-*/test_lab.py -q
/opsx:archive
```
