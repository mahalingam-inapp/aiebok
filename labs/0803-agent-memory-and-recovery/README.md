...
- Book 8: Agent Systems, chapter 3
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Agent Memory and Recovery* (normal, boundary, adversarial) in `specs/lab-0803.yaml` or as pytest cases in `test_lab.py`. Goal: Persist and resume an interrupted multi-step run.
...
```text
Read specs/lab-0803.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 8.3 — Agent Memory and Recovery
```
...
```text
/opsx:apply
python -m pytest labs/0803-*/test_lab.py -q
/opsx:archive
```
