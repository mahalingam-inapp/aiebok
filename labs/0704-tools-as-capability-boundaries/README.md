...
- Book 7: Reasoning and Tool Use, chapter 4
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Tools as Capability Boundaries* (normal, boundary, adversarial) in `specs/lab-0704.yaml` or as pytest cases in `test_lab.py`. Goal: Wrap a read-only API as a typed tool and fuzz its arguments.
...
```text
Read specs/lab-0704.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 7.4 — Tools as Capability Boundaries
```
...
```text
/opsx:apply
python -m pytest labs/0704-*/test_lab.py -q
/opsx:archive
```
