...
- Book 8: Agent Systems, chapter 6
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Operating Long-Running Agents* (normal, boundary, adversarial) in `specs/lab-0806.yaml` or as pytest cases in `test_lab.py`. Goal: Create an SLO and runbook for a day-long agent workflow.
...
```text
Read specs/lab-0806.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 8.6 — Operating Long-Running Agents
```
...
```text
/opsx:apply
python -m pytest labs/0806-*/test_lab.py -q
/opsx:archive
```
