...
- Book 4: Transformers and Foundation Models, chapter 3
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *The Transformer Block* (normal, boundary, adversarial) in `specs/lab-0403.yaml` or as pytest cases in `test_lab.py`. Goal: Assemble one transformer block and test tensor shapes.
...
```text
Read specs/lab-0403.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 4.3 — The Transformer Block
```
...
```text
/opsx:apply
python -m pytest labs/0403-*/test_lab.py -q
/opsx:archive
```
