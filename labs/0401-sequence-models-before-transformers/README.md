...
- Book 4: Transformers and Foundation Models, chapter 1
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Sequence Models Before Transformers* (normal, boundary, adversarial) in `specs/lab-0401.yaml` or as pytest cases in `test_lab.py`. Goal: Train an n-gram model and inspect where local context fails.
...
```text
Read specs/lab-0401.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 4.1 — Sequence Models Before Transformers
```
...
```text
/opsx:apply
python -m pytest labs/0401-*/test_lab.py -q
/opsx:archive
```
