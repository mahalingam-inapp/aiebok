...
- Book 3: Language and Representation, chapter 6
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Embedding Systems in Production* (normal, boundary, adversarial) in `specs/lab-0306.yaml` or as pytest cases in `test_lab.py`. Goal: Create a retrieval evaluation set with realistic queries and hard negatives.
...
```text
Read specs/lab-0306.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 3.6 — Embedding Systems in Production
```
...
```text
/opsx:apply
python -m pytest labs/0306-*/test_lab.py -q
/opsx:archive
```
