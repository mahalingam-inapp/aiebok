...
- Book 4: Transformers and Foundation Models, chapter 5
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Inference and Sampling* (normal, boundary, adversarial) in `specs/lab-0405.yaml` or as pytest cases in `test_lab.py`. Goal: Build a sampling playground and compare decoding strategies.
...
```text
Read specs/lab-0405.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 4.5 — Inference and Sampling
```
...
```text
/opsx:apply
python -m pytest labs/0405-*/test_lab.py -q
/opsx:archive
```
