...
- Book 6: Knowledge and Retrieval Systems, chapter 5
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *RAG Generation and Citations* (normal, boundary, adversarial) in `specs/lab-0605.yaml` or as pytest cases in `test_lab.py`. Goal: Build a citation validator that checks claim-to-source alignment.
...
```text
Read specs/lab-0605.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 6.5 — RAG Generation and Citations
```
...
```text
/opsx:apply
python -m pytest labs/0605-*/test_lab.py -q
/opsx:archive
```
