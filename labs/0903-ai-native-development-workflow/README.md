...
- Book 9: AI Software and Product Engineering, chapter 3
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *AI-Native Development Workflow* (normal, boundary, adversarial) in `specs/lab-0903.yaml` or as pytest cases in `test_lab.py`. Goal: Run a bounded repository task with two assistants and compare review burden.
...
```text
Read specs/lab-0903.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 9.3 — AI-Native Development Workflow
```
...
```text
/opsx:apply
python -m pytest labs/0903-*/test_lab.py -q
/opsx:archive
```
