...
- Book 2: Machine Learning Systems, chapter 6
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *The ML Lifecycle* (normal, boundary, adversarial) in `specs/lab-0206.yaml` or as pytest cases in `test_lab.py`. Goal: Write a release checklist and a rollback plan for a prediction service.
...
```text
Read specs/lab-0206.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 2.6 — The ML Lifecycle
```
...
```text
/opsx:apply
python -m pytest labs/0206-*/test_lab.py -q
/opsx:archive
```
