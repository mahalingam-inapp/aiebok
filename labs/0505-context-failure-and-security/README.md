...
- Book 5: Prompt and Context Engineering, chapter 5
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Context Failure and Security* (normal, boundary, adversarial) in `specs/lab-0505.yaml` or as pytest cases in `test_lab.py`. Goal: Attack a context pipeline with malicious retrieved text and test defenses.
...
```text
Read specs/lab-0505.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 5.5 — Context Failure and Security
```
...
```text
/opsx:apply
python -m pytest labs/0505-*/test_lab.py -q
/opsx:archive
```
