...
- Book 2: Machine Learning Systems, chapter 1
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Problems, Data, and Baselines* (normal, boundary, adversarial) in `specs/lab-0201.yaml` or as pytest cases in `test_lab.py`. Goal: Create a dataset split that respects time and entity boundaries.
...
```text
Read specs/lab-0201.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 2.1 — Problems, Data, and Baselines
```
...
```text
/opsx:apply
python -m pytest labs/0201-*/test_lab.py -q
/opsx:archive
```
