...
- Book 8: Agent Systems, chapter 1
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Agent or Workflow?* (normal, boundary, adversarial) in `specs/lab-0801.yaml` or as pytest cases in `test_lab.py`. Goal: Model the same task as a workflow and as an agent, then compare.
...
```text
Read specs/lab-0801.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 8.1 — Agent or Workflow?
```
...
```text
/opsx:apply
python -m pytest labs/0801-*/test_lab.py -q
/opsx:archive
```
