...
- Book 5: Prompt and Context Engineering, chapter 2
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Structured Generation* (normal, boundary, adversarial) in `specs/lab-0502.yaml` or as pytest cases in `test_lab.py`. Goal: Build an invoice extractor with schema validation and adversarial inputs.
...
```text
Read specs/lab-0502.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 5.2 — Structured Generation
```
...
```text
/opsx:apply
python -m pytest labs/0502-*/test_lab.py -q
/opsx:archive
```
