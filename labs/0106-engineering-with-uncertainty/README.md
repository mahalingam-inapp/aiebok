...
- Book 1: Foundations of Intelligence, chapter 6
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Engineering with Uncertainty* (normal, boundary, adversarial) in `specs/lab-0106.yaml` or as pytest cases in `test_lab.py`. Goal: Design a decision policy for a high-cost false-positive scenario.
...
```text
Read specs/lab-0106.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 1.6 — Engineering with Uncertainty
```
...
```text
/opsx:apply
python -m pytest labs/0106-*/test_lab.py -q
/opsx:archive
```
