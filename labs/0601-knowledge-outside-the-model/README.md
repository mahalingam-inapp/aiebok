...
- Book 6: Knowledge and Retrieval Systems, chapter 1
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Knowledge Outside the Model* (normal, boundary, adversarial) in `specs/lab-0601.yaml` or as pytest cases in `test_lab.py`. Goal: Classify ten requirements by the correct knowledge mechanism.
...
```text
Read specs/lab-0601.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 6.1 — Knowledge Outside the Model
```
...
```text
/opsx:apply
python -m pytest labs/0601-*/test_lab.py -q
/opsx:archive
```
