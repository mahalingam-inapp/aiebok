...
- Book 12: Cloud and Enterprise AI Architecture, chapter 2
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Identity, Data, and Trust Boundaries* (normal, boundary, adversarial) in `specs/lab-1202.yaml` or as pytest cases in `test_lab.py`. Goal: Threat-model an enterprise assistant across trust boundaries.
...
```text
Read specs/lab-1202.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 12.2 — Identity, Data, and Trust Boundaries
```
...
```text
/opsx:apply
python -m pytest labs/1202-*/test_lab.py -q
/opsx:archive
```
