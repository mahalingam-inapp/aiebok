...
- Book 8: Agent Systems, chapter 2
- Python 3.10+
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *The Agent Loop* (normal, boundary, adversarial) in `specs/lab-0802.yaml` or as pytest cases in `test_lab.py`. Goal: Extend the included agent loop with failures and checkpointing.
...
```text
Read specs/lab-0802.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 8.2 — The Agent Loop
```
...
```text
/opsx:apply
python -m pytest labs/0802-*/test_lab.py -q
/opsx:archive
```
