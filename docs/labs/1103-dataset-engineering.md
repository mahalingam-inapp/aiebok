...
Create a data card and contamination check for a small dataset.

## Prerequisites

Book [Training, Serving, and AI Operations](../books/11-training-serving-and-ai-operations/index.md), chapter 3.

## Run

```bash
python labs/1103-dataset-engineering/main.py
python -m pytest labs/1103-dataset-engineering/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Dataset Engineering* (normal, boundary, adversarial) in `specs/lab-1103.yaml` or as pytest cases in `test_lab.py`. Goal: Create a data card and contamination check for a small dataset.
...
```text
Read specs/lab-1103.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 11.3 — Dataset Engineering
```
...
```text
/opsx:apply
python -m pytest labs/1103-*/test_lab.py -q
/opsx:archive
```
