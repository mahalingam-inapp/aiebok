...
Split a research task across workers and compare with one-agent parallel tools.

## Prerequisites

Book [Agent Systems](../books/08-agent-systems/index.md), chapter 5.

## Run

```bash
python labs/0805-multi-agent-systems/main.py
python -m pytest labs/0805-multi-agent-systems/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Multi-Agent Systems* (normal, boundary, adversarial) in `specs/lab-0805.yaml` or as pytest cases in `test_lab.py`. Goal: Split a research task across workers and compare with one-agent parallel tools.
...
```text
Read specs/lab-0805.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 8.5 — Multi-Agent Systems
```
...
```text
/opsx:apply
python -m pytest labs/0805-*/test_lab.py -q
/opsx:archive
```
