...
Build a planner that outputs a validated dependency graph.

## Prerequisites

Book [Reasoning and Tool Use](../books/07-reasoning-and-tool-use/index.md), chapter 2.

## Run

```bash
python labs/0702-planning/main.py
python -m pytest labs/0702-planning/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Planning* (normal, boundary, adversarial) in `specs/lab-0702.yaml` or as pytest cases in `test_lab.py`. Goal: Build a planner that outputs a validated dependency graph.
...
```text
Read specs/lab-0702.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 7.2 — Planning
```
...
```text
/opsx:apply
python -m pytest labs/0702-*/test_lab.py -q
/opsx:archive
```
