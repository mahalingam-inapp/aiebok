...
Implement breadth-first search and A* on the same maze.

## Prerequisites

Book [Foundations of Intelligence](../books/01-foundations-of-intelligence/index.md), chapter 3.

## Run

```bash
python labs/0103-search-planning-and-decisions/main.py
python -m pytest labs/0103-search-planning-and-decisions/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Search, Planning, and Decisions* (normal, boundary, adversarial) in `specs/lab-0103.yaml` or as pytest cases in `test_lab.py`. Goal: Implement breadth-first search and A* on the same maze.
...
```text
Read specs/lab-0103.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 1.3 — Search, Planning, and Decisions
```
...
```text
/opsx:apply
python -m pytest labs/0103-*/test_lab.py -q
/opsx:archive
```
