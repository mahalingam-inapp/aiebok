...
Create a capability map for a familiar human task.

## Prerequisites

Book [Foundations of Intelligence](../books/01-foundations-of-intelligence/index.md), chapter 1.

## Run

```bash
python labs/0101-what-intelligence-means/main.py
python -m pytest labs/0101-what-intelligence-means/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *What Intelligence Means* (normal, boundary, adversarial) in `specs/lab-0101.yaml` or as pytest cases in `test_lab.py`. Goal: Create a capability map for a familiar human task.
...
```text
Read specs/lab-0101.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 1.1 — What Intelligence Means
```
...
```text
/opsx:apply
python -m pytest labs/0101-*/test_lab.py -q
/opsx:archive
```
