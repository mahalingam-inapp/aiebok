...
Solve a constraint problem with explicit state search.

## Prerequisites

Book [Reasoning and Tool Use](../books/07-reasoning-and-tool-use/index.md), chapter 1.

## Run

```bash
python labs/0701-reasoning-as-search/main.py
python -m pytest labs/0701-reasoning-as-search/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Reasoning as Search* (normal, boundary, adversarial) in `specs/lab-0701.yaml` or as pytest cases in `test_lab.py`. Goal: Solve a constraint problem with explicit state search.
...
```text
Read specs/lab-0701.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 7.1 — Reasoning as Search
```
...
```text
/opsx:apply
python -m pytest labs/0701-*/test_lab.py -q
/opsx:archive
```
