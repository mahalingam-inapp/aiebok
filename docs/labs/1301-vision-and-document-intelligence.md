...
Extract fields from documents and evaluate field and page-level accuracy.

## Prerequisites

Book [Multimodal and Frontier Systems](../books/13-multimodal-and-frontier-systems/index.md), chapter 1.

## Run

```bash
python labs/1301-vision-and-document-intelligence/main.py
python -m pytest labs/1301-vision-and-document-intelligence/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Vision and Document Intelligence* (normal, boundary, adversarial) in `specs/lab-1301.yaml` or as pytest cases in `test_lab.py`. Goal: Extract fields from documents and evaluate field and page-level accuracy.
...
```text
Read specs/lab-1301.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 13.1 — Vision and Document Intelligence
```
...
```text
/opsx:apply
python -m pytest labs/1301-*/test_lab.py -q
/opsx:archive
```
