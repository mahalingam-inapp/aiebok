...
Build a failure attribution matrix for a RAG system.

## Prerequisites

Book [Evaluation, Safety, and Governance](../books/10-evaluation-safety-and-governance/index.md), chapter 3.

## Run

```bash
python labs/1003-evaluation-by-system-stage/main.py
python -m pytest labs/1003-evaluation-by-system-stage/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Evaluation by System Stage* (normal, boundary, adversarial) in `specs/lab-1003.yaml` or as pytest cases in `test_lab.py`. Goal: Build a failure attribution matrix for a RAG system.
...
```text
Read specs/lab-1003.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 10.3 — Evaluation by System Stage
```
...
```text
/opsx:apply
python -m pytest labs/1003-*/test_lab.py -q
/opsx:archive
```
