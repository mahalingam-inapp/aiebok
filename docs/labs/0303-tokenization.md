...
Write a toy byte-pair tokenizer and compare segmentations.

## Prerequisites

Book [Language and Representation](../books/03-language-and-representation/index.md), chapter 3.

## Run

```bash
python labs/0303-tokenization/main.py
python -m pytest labs/0303-tokenization/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Tokenization* (normal, boundary, adversarial) in `specs/lab-0303.yaml` or as pytest cases in `test_lab.py`. Goal: Write a toy byte-pair tokenizer and compare segmentations.
...
```text
Read specs/lab-0303.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 3.3 — Tokenization
```
...
```text
/opsx:apply
python -m pytest labs/0303-*/test_lab.py -q
/opsx:archive
```
