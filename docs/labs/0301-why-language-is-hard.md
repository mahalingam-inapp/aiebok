...
Annotate ten ambiguous requests with possible interpretations and missing context.

## Prerequisites

Book [Language and Representation](../books/03-language-and-representation/index.md), chapter 1.

## Run

```bash
python labs/0301-why-language-is-hard/main.py
python -m pytest labs/0301-why-language-is-hard/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Why Language Is Hard* (normal, boundary, adversarial) in `specs/lab-0301.yaml` or as pytest cases in `test_lab.py`. Goal: Annotate ten ambiguous requests with possible interpretations and missing context.
...
```text
Read specs/lab-0301.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 3.1 — Why Language Is Hard
```
...
```text
/opsx:apply
python -m pytest labs/0301-*/test_lab.py -q
/opsx:archive
```
