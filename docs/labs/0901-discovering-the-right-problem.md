# Lab 9.1 — Discovering the Right Problem

## Objective

Write a problem brief with a non-AI alternative.

## Prerequisites

Book [AI Software and Product Engineering](../books/09-ai-software-and-product-engineering/index.md), chapter 1.

## Run

```bash
python labs/0901-discovering-the-right-problem/main.py
python -m pytest labs/0901-discovering-the-right-problem/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Discovering the Right Problem* (normal, boundary, adversarial) in `specs/lab-0901.yaml` or as pytest cases in `test_lab.py`. Goal: Write a problem brief with a non-AI alternative.
...
```text
Read specs/lab-0901.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 9.1 — Discovering the Right Problem
```
...
```text
/opsx:apply
python -m pytest labs/0901-*/test_lab.py -q
/opsx:archive
```
