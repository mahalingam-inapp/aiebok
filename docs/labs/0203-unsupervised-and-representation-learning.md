...
Book [Machine Learning Systems](../books/02-machine-learning-systems/index.md), chapter 3.

## Run

```bash
python labs/0203-unsupervised-and-representation-learning/main.py
python -m pytest labs/0203-unsupervised-and-representation-learning/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Unsupervised and Representation Learning* (normal, boundary, adversarial) in `specs/lab-0203.yaml` or as pytest cases in `test_lab.py`. Goal: Cluster a dataset, visualize it, and explain why clusters are not automatically meaningful categories.
...
```text
Read specs/lab-0203.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 2.3 — Unsupervised and Representation Learning
```
...
```text
/opsx:apply
python -m pytest labs/0203-*/test_lab.py -q
/opsx:archive
```
