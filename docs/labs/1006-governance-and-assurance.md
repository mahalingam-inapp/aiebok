...
Book [Evaluation, Safety, and Governance](../books/10-evaluation-safety-and-governance/index.md), chapter 6.

## Run

```bash
python labs/1006-governance-and-assurance/main.py
python -m pytest labs/1006-governance-and-assurance/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Governance and Assurance* (normal, boundary, adversarial) in `specs/lab-1006.yaml` or as pytest cases in `test_lab.py`. Goal: Create a lightweight governance operating model for a mid-size company.
...
```text
Read specs/lab-1006.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 10.6 — Governance and Assurance
```
...
```text
/opsx:apply
python -m pytest labs/1006-*/test_lab.py -q
/opsx:archive
```
