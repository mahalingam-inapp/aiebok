...
Train a small network and inspect gradients and learning curves.

## Prerequisites

Book [Machine Learning Systems](../books/02-machine-learning-systems/index.md), chapter 4.

## Run

```bash
python labs/0204-neural-networks/main.py
python -m pytest labs/0204-neural-networks/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Neural Networks* (normal, boundary, adversarial) in `specs/lab-0204.yaml` or as pytest cases in `test_lab.py`. Goal: Train a small network and inspect gradients and learning curves.
...
```text
Read specs/lab-0204.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 2.4 — Neural Networks
```
...
```text
/opsx:apply
python -m pytest labs/0204-*/test_lab.py -q
/opsx:archive
```
