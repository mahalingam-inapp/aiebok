...
Create an ingestion manifest and measure parse fidelity.

## Prerequisites

Book [Knowledge and Retrieval Systems](../books/06-knowledge-and-retrieval-systems/index.md), chapter 2.

## Run

```bash
python labs/0602-document-ingestion/main.py
python -m pytest labs/0602-document-ingestion/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Document Ingestion* (normal, boundary, adversarial) in `specs/lab-0602.yaml` or as pytest cases in `test_lab.py`. Goal: Create an ingestion manifest and measure parse fidelity.
...
```text
Read specs/lab-0602.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 6.2 — Document Ingestion
```
...
```text
/opsx:apply
python -m pytest labs/0602-*/test_lab.py -q
/opsx:archive
```
