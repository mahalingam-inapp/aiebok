...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Evaluation and Error Analysis* (normal, boundary, adversarial) in `specs/lab-0205.yaml` or as pytest cases in `test_lab.py`. Goal: Write an error taxonomy and compare two models with confidence intervals.
...
```text
Read specs/lab-0205.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 2.5 — Evaluation and Error Analysis
```
...
```text
/opsx:apply
python -m pytest labs/0205-*/test_lab.py -q
/opsx:archive
```
