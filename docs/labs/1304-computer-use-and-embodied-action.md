...
Design a safe browser task with confirmation and recovery.

## Prerequisites

Book [Multimodal and Frontier Systems](../books/13-multimodal-and-frontier-systems/index.md), chapter 4.

## Run

```bash
python labs/1304-computer-use-and-embodied-action/main.py
python -m pytest labs/1304-computer-use-and-embodied-action/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Computer Use and Embodied Action* (normal, boundary, adversarial) in `specs/lab-1304.yaml` or as pytest cases in `test_lab.py`. Goal: Design a safe browser task with confirmation and recovery.
...
```text
Read specs/lab-1304.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 13.4 — Computer Use and Embodied Action
```
...
```text
/opsx:apply
python -m pytest labs/1304-*/test_lab.py -q
/opsx:archive
```
