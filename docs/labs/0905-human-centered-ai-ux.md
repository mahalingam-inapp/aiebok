...
Prototype a high-risk action flow with preview and approval.

## Prerequisites

Book [AI Software and Product Engineering](../books/09-ai-software-and-product-engineering/index.md), chapter 5.

## Run

```bash
python labs/0905-human-centered-ai-ux/main.py
python -m pytest labs/0905-human-centered-ai-ux/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Human-Centered AI UX* (normal, boundary, adversarial) in `specs/lab-0905.yaml` or as pytest cases in `test_lab.py`. Goal: Prototype a high-risk action flow with preview and approval.
...
```text
Read specs/lab-0905.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 9.5 — Human-Centered AI UX
```
...
```text
/opsx:apply
python -m pytest labs/0905-*/test_lab.py -q
/opsx:archive
```
