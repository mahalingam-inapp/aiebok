...
Run the cosine and semantic-search labs, then add hybrid scoring.

## Prerequisites

Book [Language and Representation](../books/03-language-and-representation/index.md), chapter 5.

## Run

```bash
python labs/0305-similarity-and-vector-search/main.py
python -m pytest labs/0305-similarity-and-vector-search/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Similarity and Vector Search* (normal, boundary, adversarial) in `specs/lab-0305.yaml` or as pytest cases in `test_lab.py`. Goal: Run the cosine and semantic-search labs, then add hybrid scoring.
...
```text
Read specs/lab-0305.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 3.5 — Similarity and Vector Search
```
...
```text
/opsx:apply
python -m pytest labs/0305-*/test_lab.py -q
/opsx:archive
```
