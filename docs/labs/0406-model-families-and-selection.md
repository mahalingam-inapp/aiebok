...
Benchmark candidate models on a task-specific dataset.

## Prerequisites

Book [Transformers and Foundation Models](../books/04-transformers-and-foundation-models/index.md), chapter 6.

## Run

```bash
python labs/0406-model-families-and-selection/main.py
python -m pytest labs/0406-model-families-and-selection/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Model Families and Selection* (normal, boundary, adversarial) in `specs/lab-0406.yaml` or as pytest cases in `test_lab.py`. Goal: Benchmark candidate models on a task-specific dataset.
...
```text
Read specs/lab-0406.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 4.6 — Model Families and Selection
```
...
```text
/opsx:apply
python -m pytest labs/0406-*/test_lab.py -q
/opsx:archive
```
