...
Compute dot products, cosine similarity, softmax, and one gradient update by hand.

## Prerequisites

Book [Foundations of Intelligence](../books/01-foundations-of-intelligence/index.md), chapter 4.

## Run

```bash
python labs/0104-the-mathematics-engineers-need/main.py
python -m pytest labs/0104-the-mathematics-engineers-need/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *The Mathematics Engineers Need* (normal, boundary, adversarial) in `specs/lab-0104.yaml` or as pytest cases in `test_lab.py`. Goal: Compute dot products, cosine similarity, softmax, and one gradient update by hand.
...
```text
Read specs/lab-0104.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 1.4 — The Mathematics Engineers Need
```
...
```text
/opsx:apply
python -m pytest labs/0104-*/test_lab.py -q
/opsx:archive
```
