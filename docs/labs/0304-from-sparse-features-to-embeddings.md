...
Implement TF–IDF and compare it with the included vector lab.

## Prerequisites

Book [Language and Representation](../books/03-language-and-representation/index.md), chapter 4.

## Run

```bash
python labs/0304-from-sparse-features-to-embeddings/main.py
python -m pytest labs/0304-from-sparse-features-to-embeddings/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *From Sparse Features to Embeddings* (normal, boundary, adversarial) in `specs/lab-0304.yaml` or as pytest cases in `test_lab.py`. Goal: Implement TF–IDF and compare it with the included vector lab.
...
```text
Read specs/lab-0304.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 3.4 — From Sparse Features to Embeddings
```
...
```text
/opsx:apply
python -m pytest labs/0304-*/test_lab.py -q
/opsx:archive
```
