...
Load-test a local model at several concurrency levels.

## Prerequisites

Book [Training, Serving, and AI Operations](../books/11-training-serving-and-ai-operations/index.md), chapter 4.

## Run

```bash
python labs/1104-inference-infrastructure/main.py
python -m pytest labs/1104-inference-infrastructure/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Inference Infrastructure* (normal, boundary, adversarial) in `specs/lab-1104.yaml` or as pytest cases in `test_lab.py`. Goal: Load-test a local model at several concurrency levels.
...
```text
Read specs/lab-1104.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 11.4 — Inference Infrastructure
```
...
```text
/opsx:apply
python -m pytest labs/1104-*/test_lab.py -q
/opsx:archive
```
