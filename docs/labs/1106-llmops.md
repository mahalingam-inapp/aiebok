...
Instrument a request and inject provider, retrieval, and validation failures.

## Prerequisites

Book [Training, Serving, and AI Operations](../books/11-training-serving-and-ai-operations/index.md), chapter 6.

## Run

```bash
python labs/1106-llmops/main.py
python -m pytest labs/1106-llmops/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *LLMOps* (normal, boundary, adversarial) in `specs/lab-1106.yaml` or as pytest cases in `test_lab.py`. Goal: Instrument a request and inject provider, retrieval, and validation failures.
...
```text
Read specs/lab-1106.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 11.6 — LLMOps
```
...
```text
/opsx:apply
python -m pytest labs/1106-*/test_lab.py -q
/opsx:archive
```
