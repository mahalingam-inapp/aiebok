...
Draw a logical platform architecture before naming products.

## Prerequisites

Book [Cloud and Enterprise AI Architecture](../books/12-cloud-and-enterprise-ai-architecture/index.md), chapter 1.

## Run

```bash
python labs/1201-enterprise-ai-building-blocks/main.py
python -m pytest labs/1201-enterprise-ai-building-blocks/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Enterprise AI Building Blocks* (normal, boundary, adversarial) in `specs/lab-1201.yaml` or as pytest cases in `test_lab.py`. Goal: Draw a logical platform architecture before naming products.
...
```text
Read specs/lab-1201.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 12.1 — Enterprise AI Building Blocks
```
...
```text
/opsx:apply
python -m pytest labs/1201-*/test_lab.py -q
/opsx:archive
```
