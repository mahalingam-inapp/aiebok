...
Implement two patterns and measure coordination overhead.

## Prerequisites

Book [Agent Systems](../books/08-agent-systems/index.md), chapter 4.

## Run

```bash
python labs/0804-agent-patterns/main.py
python -m pytest labs/0804-agent-patterns/test_lab.py -q
```
...
1. **Write cases first** — Before changing `main.py`, write **three acceptance cases** for *Agent Patterns* (normal, boundary, adversarial) in `specs/lab-0804.yaml` or as pytest cases in `test_lab.py`. Goal: Implement two patterns and measure coordination overhead.
...
```text
Read specs/lab-0804.yaml (or test_lab.py cases).
...
```text
/opsx:propose Add acceptance spec for lab 8.4 — Agent Patterns
```
...
```text
/opsx:apply
python -m pytest labs/0804-*/test_lab.py -q
/opsx:archive
```
