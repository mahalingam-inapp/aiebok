# Choosing Adaptation

**Lesson ID:** `14-infrastructure-04`

## Objective

Create a decision table for ten adaptation scenarios.

## Read

- Primary: [Choosing Adaptation](../books/11-training-serving-and-ai-operations/01-choosing-adaptation.md)

## Core concepts

- [behavior versus knowledge](../concepts/cards/behavior-versus-knowledge.md)
- [prompting](../concepts/cards/prompting.md)
- [RAG](../concepts/cards/rag.md)

## Exercise

Apply the chapter insight: Choose the smallest intervention at the correct system layer.

## Practice

- Run [lab](../labs/1101-choosing-adaptation.md)

## Check yourself

1. What problem does this lesson solve that a generic LLM call does not?
2. What baseline would you compare against before adding complexity?
3. What failure mode appears first under stress?

## Unlock

Completing this lesson prepares you for the next item in your [knowledge-area path](../knowledge-areas/index.md) and related [build guides](../guides/index.md).
