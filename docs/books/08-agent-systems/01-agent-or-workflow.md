...
- Explain the problem that motivated agent or workflow?.
...
|---|---|---|
| **Agency** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Workflows** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **State Machines** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Autonomy** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Control** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A multi-step task may pause for hours and must resume without repeating side effects.

**Chapter focus:** Define agents by goal-directed action selection in a loop and contrast them with deterministic workflows and single tool calls.
...
2. Locate where agency and workflows enter the book-level visual above.
...
The design question is: **What evidence would show that agent or workflow? addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if agency were removed from the system?
2. Which observation would distinguish a failure in workflows from a failure in control?
