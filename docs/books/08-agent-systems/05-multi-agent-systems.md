...
- Explain the problem that motivated multi-agent systems.
...
|---|---|---|
| **Delegation** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Coordination** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Shared State** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Consensus** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Role Isolation** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A multi-step task may pause for hours and must resume without repeating side effects.

**Chapter focus:** Study delegation, role boundaries, communication, shared state, consensus, conflict, security, and why many tasks do not need multiple agents.
...
2. Locate where delegation and coordination enter the book-level visual above.
...
The design question is: **What evidence would show that multi-agent systems addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if delegation were removed from the system?
2. Which observation would distinguish a failure in coordination from a failure in role isolation?
