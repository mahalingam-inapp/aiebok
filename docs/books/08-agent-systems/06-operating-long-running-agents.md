...
- Explain the problem that motivated operating long-running agents.
...
|---|---|---|
| **Durable Execution** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Queues** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Leases** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
...
| **Cancellation** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A multi-step task may pause for hours and must resume without repeating side effects.

**Chapter focus:** Design durable orchestration, queues, scheduling, leases, approvals, monitoring, incident response, and safe cancellation.
...
2. Locate where durable execution and queues enter the book-level visual above.
...
The design question is: **What evidence would show that operating long-running agents addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if durable execution were removed from the system?
2. Which observation would distinguish a failure in queues from a failure in cancellation?
