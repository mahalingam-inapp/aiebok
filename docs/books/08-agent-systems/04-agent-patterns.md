...
- Explain the problem that motivated agent patterns.
...
|---|---|---|
| **Planner-Executor** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Supervisor-Worker** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Reviewer** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
...
| **Approval Gates** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A multi-step task may pause for hours and must resume without repeating side effects.

**Chapter focus:** Apply planner–executor, supervisor–worker, reviewer, evaluator–optimizer, routing, and human-approval patterns.
...
2. Locate where planner-executor and supervisor-worker enter the book-level visual above.
...
The design question is: **What evidence would show that agent patterns addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if planner-executor were removed from the system?
2. Which observation would distinguish a failure in supervisor-worker from a failure in approval gates?
