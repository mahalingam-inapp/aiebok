...
- Explain the problem that motivated document ingestion.
...
|---|---|---|
| **Parsing** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
...
| **Chunking** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Metadata** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
...
**Chapter focus:** Preserve provenance while parsing documents, OCR, tables, images, metadata, permissions, versions, and deletions.
...
2. Locate where parsing and OCR enter the book-level visual above.
...
The design question is: **What evidence would show that document ingestion addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if parsing were removed from the system?
2. Which observation would distinguish a failure in OCR from a failure in provenance?
