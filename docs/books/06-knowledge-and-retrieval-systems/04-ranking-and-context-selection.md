...
- Explain the problem that motivated ranking and context selection.
...
|---|---|---|
| **Reciprocal Rank Fusion** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Rerankers** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Diversity** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Deduplication** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Context Packing** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** An enterprise assistant must answer from authorized policies and cite the exact passages used.

**Chapter focus:** Use fusion, reranking, diversity, deduplication, compression, and token-aware packing.
...
2. Locate where reciprocal rank fusion and rerankers enter the book-level visual above.
...
The design question is: **What evidence would show that ranking and context selection addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if reciprocal rank fusion were removed from the system?
2. Which observation would distinguish a failure in rerankers from a failure in context packing?
