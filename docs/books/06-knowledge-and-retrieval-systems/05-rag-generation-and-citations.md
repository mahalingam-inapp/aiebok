...
- Explain the problem that motivated rag generation and citations.
...
|---|---|---|
| **Grounded Generation** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Abstention** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Citation Precision** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Faithfulness** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Answer Validation** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** An enterprise assistant must answer from authorized policies and cite the exact passages used.

**Chapter focus:** Construct grounded prompts, handle missing evidence, attribute claims, validate citations, and avoid unsupported synthesis.
...
2. Locate where grounded generation and abstention enter the book-level visual above.
...
The design question is: **What evidence would show that rag generation and citations addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if grounded generation were removed from the system?
2. Which observation would distinguish a failure in abstention from a failure in answer validation?
