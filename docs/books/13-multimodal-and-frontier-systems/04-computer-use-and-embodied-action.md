...
- Explain the problem that motivated computer use and embodied action.
...
|---|---|---|
| **Computer Use** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Visual Grounding** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Action Spaces** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Recovery** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Robotics Interfaces** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A document system must combine tables, charts, and text without losing source provenance.

**Chapter focus:** Model perception–action loops, UI grounding, coordinate and semantic actions, recovery, permissions, and physical-world constraints.
...
2. Locate where computer use and visual grounding enter the book-level visual above.
...
The design question is: **What evidence would show that computer use and embodied action addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if computer use were removed from the system?
2. Which observation would distinguish a failure in visual grounding from a failure in robotics interfaces?
