...
- Explain the problem that motivated what intelligence means.
...
|---|---|---|
| **Goal-Directed Behavior** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Rational Agents** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Bounded Rationality** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Capability Decomposition** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Feedback** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A support team must route incidents without mistaking fluent descriptions for reliable decisions.

**Chapter focus:** Treat intelligence as a collection of capabilities rather than a mystical substance. Separate perception, representation, memory, learning, reasoning, planning, action, and adaptation.
...
2. Locate where goal-directed behavior and rational agents enter the book-level visual above.
...
The design question is: **What evidence would show that what intelligence means addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if goal-directed behavior were removed from the system?
2. Which observation would distinguish a failure in rational agents from a failure in feedback?
