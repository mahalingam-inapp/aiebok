...
- Explain the problem that motivated attention.
...
|---|---|---|
| **Queries** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Keys** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Values** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Scaled Dot Product** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Attention Masks** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A team must explain why decoding settings change model output and latency.

**Chapter focus:** Build attention from queries, keys, values, similarity scores, normalization, and weighted aggregation.
...
2. Locate where queries and keys enter the book-level visual above.
...
The design question is: **What evidence would show that attention addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if queries were removed from the system?
2. Which observation would distinguish a failure in keys from a failure in attention masks?
