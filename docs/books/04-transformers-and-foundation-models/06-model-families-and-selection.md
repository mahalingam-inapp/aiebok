...
- Explain the problem that motivated model families and selection.
...
|---|---|---|
| **Instruction Tuning** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Reasoning Models** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Multimodal Models** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Open Weights** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Model Routing** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A team must explain why decoding settings change model output and latency.

**Chapter focus:** Compare base, instruction, reasoning, code, embedding, reranking, reward, safety, speech, vision, and diffusion models.
...
2. Locate where instruction tuning and reasoning models enter the book-level visual above.
...
The design question is: **What evidence would show that model families and selection addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if instruction tuning were removed from the system?
2. Which observation would distinguish a failure in reasoning models from a failure in model routing?
