...
- Explain the problem that motivated supervised learning.
...
|---|---|---|
| **Regression** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Classification** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Loss Functions** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Regularization** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Optimization** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A lender needs a prediction service whose errors can be explained across customer groups.

**Chapter focus:** Understand regression and classification as function approximation under a chosen loss. Connect linear models, trees, and neural networks through their assumptions.
...
2. Locate where regression and classification enter the book-level visual above.
...
The design question is: **What evidence would show that supervised learning addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if regression were removed from the system?
2. Which observation would distinguish a failure in classification from a failure in optimization?
