...
- Explain the problem that motivated identity, data, and trust boundaries.
...
|---|---|---|
| **Identity** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Authorization** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Multi-Tenancy** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Data Residency** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Audit** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** An architect must implement the same governed AI capability on different cloud providers.

**Chapter focus:** Apply authentication, authorization, tenancy, secrets, encryption, residency, lineage, and audit to AI data flows.
...
2. Locate where identity and authorization enter the book-level visual above.
...
The design question is: **What evidence would show that identity, data, and trust boundaries addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if identity were removed from the system?
2. Which observation would distinguish a failure in authorization from a failure in audit?
