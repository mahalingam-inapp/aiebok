...
- Explain the problem that motivated verification and critique.
...
|---|---|---|
| **Verifiers** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Critique** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Self-Consistency** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Best-Of-N** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Tests** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A research workflow must plan, call tools, and reject unsupported conclusions.

**Chapter focus:** Use deterministic checks, tests, rubrics, critics, self-consistency, best-of-N, and external evidence.
...
2. Locate where verifiers and critique enter the book-level visual above.
...
The design question is: **What evidence would show that verification and critique addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if verifiers were removed from the system?
2. Which observation would distinguish a failure in critique from a failure in tests?
