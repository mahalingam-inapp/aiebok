...
- Explain the problem that motivated reasoning as search.
...
|---|---|---|
| **Decomposition** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Search** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Backtracking** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
...
| **Termination** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A research workflow must plan, call tools, and reject unsupported conclusions.

**Chapter focus:** Separate recall from deliberate search and study decomposition, candidate generation, backtracking, and stopping.
...
2. Locate where decomposition and search enter the book-level visual above.
...
The design question is: **What evidence would show that reasoning as search addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if decomposition were removed from the system?
2. Which observation would distinguish a failure in search from a failure in termination?
