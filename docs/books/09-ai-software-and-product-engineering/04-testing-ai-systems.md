...
- Explain the problem that motivated testing ai systems.
...
|---|---|---|
| **Unit Tests** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Contract Tests** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Eval Datasets** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Adversarial Tests** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Release Gates** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A product team must convert a vague AI feature request into testable release evidence.

**Chapter focus:** Combine unit, contract, integration, scenario, regression, adversarial, and human tests across deterministic and probabilistic components.
...
2. Locate where unit tests and contract tests enter the book-level visual above.
...
The design question is: **What evidence would show that testing ai systems addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if unit tests were removed from the system?
2. Which observation would distinguish a failure in contract tests from a failure in release gates?
