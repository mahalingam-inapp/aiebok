...
- Explain the problem that motivated tokenization.
...
|---|---|---|
| **Vocabulary** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Subwords** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Bpe** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Sentencepiece** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Token Budgets** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** Employees search for policies using vocabulary different from the source documents.

**Chapter focus:** Understand character, word, and subword tokenization; BPE, WordPiece, and SentencePiece; and the impact on cost, latency, languages, and code.
...
2. Locate where vocabulary and subwords enter the book-level visual above.
...
The design question is: **What evidence would show that tokenization addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if vocabulary were removed from the system?
2. Which observation would distinguish a failure in subwords from a failure in token budgets?
