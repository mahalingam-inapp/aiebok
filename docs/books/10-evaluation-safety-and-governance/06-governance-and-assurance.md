...
- Explain the problem that motivated governance and assurance.
...
|---|---|---|
| **Ai Inventory** | establishes the first representation or decision boundary | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Risk Tiers** | adds the main transformation or comparison | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Model Cards** | connects the mechanism to the surrounding system | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Audit Evidence** | controls quality, efficiency, or behavior | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
| **Incident Response** | exposes an important operating constraint or failure mode | Define inputs and outputs; construct a minimal example; identify one invalid assumption. |
## Worked example

**Book scenario:** A high-impact assistant may pass average quality while failing a safety-critical user slice.

**Chapter focus:** Define ownership, inventory, risk tiers, policies, approvals, audit evidence, incidents, exceptions, vendor review, and retirement.
...
2. Locate where AI inventory and risk tiers enter the book-level visual above.
...
The design question is: **What evidence would show that governance and assurance addresses this chapter's problem better than the baseline?** Answer with measured observations rather than intuition alone.
...
1. What problem would remain if AI inventory were removed from the system?
2. Which observation would distinguish a failure in risk tiers from a failure in incident response?
